...
void en_Flare(sfVector2f _pos, float _lifetime) {
	...
	float spd = FLARE_SPD * Bars(4) / _lifetime;
...
void en_flare_Render(struct EnData* _en) {
	...
	va_DrawPolygonReg(VA_LINE, NULL, 5, _en->pos, 80.f, _en->dataGl.rot, colorBase);

	/// For the warning circle
	colorBase.a = 96;
	colorBlink.a = 24;
	va_DrawCircle(VA_LINE, NULL, _en->dataGl.pos_target, 250.f, colorBase);
	va_DrawCircle(VA_TRI, NULL, _en->dataGl.pos_target, itp_Float(0.f, 250.f, clamp(_en->lifetime / _en->dataGl.fall_time, 0.f, 1.f), itp_Square), colorBlink);
}
